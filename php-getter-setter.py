...
class Prefs:
	...
	def load(self):
		...
		self.data['registerTemplates'] = settings.get('register_templates', [])
...
class Variable(object):
	...
	def isPrivateVisibility(self):
		visibility = self.visibility

		return (visibility == 'private' or visibility == 'protected')

	def isUsingPrivateStyle(self):
		name = self.name

		return (name[0] == '_' and self.isPrivateVisibility())

	def canUsePrivateVisibility(self):
		return (self.isPrivateVisibility() and not self.Prefs.get('ignoreVisibility'))

	def usePrivateStyle(self):
		return (self.isUsingPrivateStyle() and self.canUsePrivateVisibility())

	def getVisibilityPrefix(self):
		Prefix = ''

		if (self.usePrivateStyle()):
			Prefix = '_'

		return Prefix

	def getFunctionVisibility(self):
		visibility = self.getVisibility()

		if (not self.canUsePrivateVisibility()):
			visibility = 'public'
...
class Base(sublime_plugin.TextCommand):
	...
	def generateFunctionCode(self, template, variable):
		substitutions = {
			...
			"visibility": variable.getFunctionVisibility(),
	...
	def isPhpSyntax(self):
		return re.search(r".*PHP\.sublime-syntax", self.view.settings().get('syntax')) is not None
...
"""
	 ...
	 *
	 * @param %(type)s $%(param)s the %(humanName)s
	 *
	 * @return self
	 */
	%(visibility)s function %(visibilityPrefix)s%(setterPrefix)s%(normalizedName)s(%(typeHint)s $%(param)s)
...
"""
	 ...
	 * Sets the %(description)s.
	 *
	 * @param %(type)s $%(param)s the %(name)s
	 *
	 * @return self
	 */
	%(visibility)s function %(visibilityPrefix)s%(setterPrefix)s%(normalizedName)s(%(typeHint)s $%(param)s)
